...
namespace DanksMod.Projectiles
{
	public class SunkenPrismBeam : ModProjectile
	{
		...
		private const float MaxBeamSpread = 1f;
		...
		private const float BeamHueVariance = 0.01f;
		private const float BeamColorSaturation = 1f;
		private const float BeamColorLightness = 0743f;
		...
		{
			DisplayName.SetDefault("Sunken Prism Beam");
		}
		...
		public override void AI()
		{
			...
			if (projectile.type != ModContent.ProjectileType<SunkenPrismBeam>() || !hostPrism.active || hostPrism.type != ModContent.ProjectileType<SunkenPrismHoldout>())
			{
				projectile.Kill();
				return;
			}
			...
			float chargeRatio = MathHelper.Clamp(hostPrism.ai[0] / SunkenPrismHoldout.MaxCharge, 0f, 1f);
			...
			projectile.friendly = hostPrism.ai[0] > SunkenPrismHoldout.DamageStart;

			// This offset is used to make each individual beam orient differently based on its Beam ID.
			float beamIdOffset = BeamID - SunkenPrismHoldout.NumBeams / 2f + 0.5f;
			float beamSpread;
			float spinRate;
			...
			float deviationAngle = (hostPrism.ai[0] + beamIdOffset * spinRate) / (spinRate * SunkenPrismHoldout.NumBeams) * MathHelper.TwoPi;
		...
		private Color GetOuterBeamColor()
		{
			...
			float hue = (BeamID / SunkenPrismHoldout.NumBeams) % BeamHueVariance + BeamColorHue;
		...
		public override void OnHitNPC(NPC target, int damage, float knockback, bool crit)
		{
			...
			{
				target.AddBuff(ModContent.BuffType<CalamityMod.Buffs.DamageOverTime.AstralInfectionDebuff>(), 120);
			}
		}
	}
}